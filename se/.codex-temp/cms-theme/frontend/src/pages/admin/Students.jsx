...
export default function Students() {
  ...
  const canAddStudent = can('admin_students_create', ['super_admin', 'admin', 'admission_staff']);
  const canEditStudent = can('admin_students_edit', ['super_admin', 'admin', 'admission_staff', 'class_teacher']);
  const canDeactivateStudent = can('admin_students_deactivate', ['super_admin', 'admin']);
  const canPromoteOrGenerateRollNos = can('admin_students_promote', ['super_admin', 'admin']);
  const canGenerateRollNos = can('admin_roll_numbers_generate', ['super_admin', 'admin', 'class_teacher']);
      ...
      <td className="table-cell"
        ...
        <div className="flex items-center gap-2 whitespace-nowrap">
          <button
            type="button"
            onClick={() => navigate(`/admin/students/${s._id}`)}
            className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-300 bg-white text-slate-600 transition hover:bg-slate-900 hover:text-white"
