...
const getDashboardTarget = role => {
  if (role === 'librarian') return '/admin/library/dashboard';
  if (role === 'hostel_warden') return '/admin/hostel/dashboard';
  if (role === 'class_teacher') return '/admin/students';
  return '/admin';
};
  ...
  const filteredNav = useMemo(() => {
    return NAV_ENTRIES.reduce((items, entry) => {
      if (entry.type === 'link') {
        ...
        items.push(
          entry.key === 'dashboard'
            ? { ...entry, to: getDashboardTarget(user?.role), exact: true }
            : entry
        );
        return items;
      }

      const children = (entry.children || []).filter(child => !child.capability || can(child.capability, []));
