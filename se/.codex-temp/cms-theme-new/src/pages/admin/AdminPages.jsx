...
const isStaffRoleAvailable = (role, hasFeature) => {
  const normalizedRole = String(role || '').trim().toLowerCase();
  if (!normalizedRole) return false;
  if (['shop_operator', 'canteen_operator', 'shop_canteen_operator_admin', 'shop_operator_admin'].includes(normalizedRole)) {
    return hasFeature('commerce_pos');
  }
  if (normalizedRole === 'librarian') {
    return hasFeature('library');
  }
  if (normalizedRole === 'accountant') {
    return hasFeature('finance');
  }
  if (normalizedRole === 'hostel_warden') {
    return hasFeature('hostel_leave') || hasFeature('hostel_checkin');
  }
  return true;
};
          ...
          <div>
            ...
            {isClassTeacher && (
              <p className="mt-2 text-xs text-gray-500">
                Class advisor circulars are restricted to your assigned course and can be sent only to students and parents.
              </p>
            )}
          </div>
...
export function StaffManagement() {
  const { user } = useAuth();
  const { roleDefinitions, hasFeature } = useAppSettings();
  ...
  const initialStaffForm = { name: '', phone: '', email: '', password: '', role: '', department: '' };
  ...
  const add = async e => {
    ...
    try {
      ...
      if (!form.role) {
        toast.error('Select a staff role before saving');
        return;
      }
  ...
  const staffRoleOptions = useMemo(
    () => (roleDefinitions || [])
      ...
      .filter(role => ['institution_owner', 'super_admin'].includes(user?.role) || !['institution_owner', 'super_admin'].includes(role.value))
      .filter(role => isStaffRoleAvailable(role.value, hasFeature))
      .map(role =>
        toSelectOption(
    ...
    [getStaffRoleLabel, hasFeature, roleDefinitions, user?.role]
  );
  const currentRoleOption = useMemo(() => {
    const normalizedRole = normalizeStaffRole(form.role);
    if (!editingStaffId || !normalizedRole) return [];
    if (staffRoleOptions.some(option => option.value === normalizedRole)) return [];
    return [
      toSelectOption(
        normalizedRole,
        `${getStaffRoleLabel(normalizedRole)} (Current Role)`,
        `${normalizedRole} ${getStaffRoleLabel(normalizedRole)}`.trim()
      ),
    ];
  }, [editingStaffId, form.role, getStaffRoleLabel, staffRoleOptions]);
          ...
          {staff.map(s => <tr key={s._id} className="hover:bg-gray-50">
            ...
            <td className="table-cell">
              <div className="flex items-center gap-2 whitespace-nowrap">
                ...
                {!['institution_owner', 'super_admin'].includes(normalizeStaffRole(s.role)) ? (
                  <button
                    type="button"
                ...
                {!['institution_owner', 'super_admin'].includes(normalizeStaffRole(s.role)) && String(s._id) !== String(user?._id) ? (
                  <button
                    type="button"
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div><label className="label">Role</label>
              <SearchableSelect
                ...
                persistentOptions={currentRoleOption}
                required
              />
            </div>
            <div>
          ...
          >
            ...
            <div className="mt-4 pt-3 border-t border-gray-100 flex items-center gap-2">
                <button
                  ...
                  className="action-icon-button action-icon-button-lg action-icon-button-primary"
              ...
              <button
                ...
                className={`action-icon-button action-icon-button-lg ${
                  c.isActive
                    ? 'action-icon-button-danger'
                    : 'action-icon-button-success'
                }`}
