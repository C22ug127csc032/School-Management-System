...
export default function ParentRegister() {
  ...
  const resolvingPortal = useCanonicalPortalRedirect(scopedPortalKey, '/parent/register');
  const { completeLogin } = useAuth();
  const { getMasterOptions } = useAppSettings();
  const [step, setStep] = useState(1);
  const [loading, setLoading] = useState(false);
  const [tenantPortalKey, setTenantPortalKey] = useState(scopedPortalKey || getStoredInstitutionPortalKey());
  ...
  const scopedInstitution = usePortalInstitution(scopedPortalKey || tenantPortalKey);
  ...
  const handleSendOTP = async event => {
    ...
    if (!String(scopedPortalKey || tenantPortalKey || '').trim()) {
    ...
    try {
      const response = await api.post('/parent/request-register-otp', {
        slug: String(scopedPortalKey || tenantPortalKey || '').trim().toLowerCase(),
        portalKey: String(scopedPortalKey || tenantPortalKey || '').trim().toLowerCase(),
  ...
  const handleRegister = async event => {
    ...
    try {
      ...
      window.location.href = getHomePathForRole('parent', String(scopedPortalKey || tenantPortalKey || '').trim().toLowerCase());
    } catch (error) {
      ...
      {step === 1 ? (
        <form onSubmit={handleSendOTP} className="space-y-4">
          {!scopedPortalKey ? (
            <div>
              ...
              <input
                ...
                value={tenantPortalKey}
                onChange={event => setTenantPortalKey(event.target.value.toLowerCase())}
                required
              />
            </div>
          ) : null}
          ...
          <p className="text-center text-sm text-text-secondary">
            Already registered?{' '}
            <Link to={scopedPortalKey ? `/${scopedPortalKey}/parent/login` : tenantPortalKey ? `/${tenantPortalKey}/parent/login` : '/parent/login'} className="font-medium text-primary-700 hover:underline">
              Login here
            </Link>
          </p>
        </form>
